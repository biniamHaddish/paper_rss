...
public class ExpandableList {
    ...
    public Set<String> expandedGroupsTitles;

    public ExpandableList(List<? extends ExpandableGroup> groups, Set<String> strings) {
        this.groups = groups;
        this.expandedGroupsTitles = strings;
        expandedGroupIndexes = new boolean[groups.size()];
        for (int i = 0; i < groups.size(); i++) {
            if (expandedGroupsTitles.contains(groups.get(i).getTitle())) {
                Log.d("ExpandableList", String.format("ExpandableList: %s has been expanded", groups.get(i).getTitle()));
                expandedGroupIndexes[i] = true;
            } else {
